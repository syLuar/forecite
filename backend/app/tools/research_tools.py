...
class StreamlinedLegalResearchTools:
        ...
        with self.driver.session() as session:
            if combine_mode.upper() == "ALL":
                ...
                for i, tag in enumerate(legal_tags):
                    conditions.append(f"ANY(existing_tag IN d.legal_tags WHERE existing_tag CONTAINS $tag_{i})")
    ...
    def analyze_citation_network(
        ...
        with self.driver.session() as session:
            ...
            result.influence_score = min(total_citations / 10.0, 1.0)  # Normalize to 0-1
    ...
    def trace_legal_precedent(
        ...
        if jurisdiction:
            chunks = [c for c in chunks if jurisdiction.lower() in c.source_document.jurisdiction.lower()]
        
        # Filter by date if specified
        if start_date:
            start_year = int(start_date[:4])
            chunks = [c for c in chunks if c.source_document.year and c.source_document.year >= start_year]
    ...
    def search_statute_references(
        ...
        with self.driver.session() as session:
            ...
            if section:
                query_parts[0] += " AND ANY(statute IN c.statutes WHERE statute CONTAINS $section)"
            ...
            for record in results:
                for holding_text in record["holdings"]:
                    ...
                    holding = LegalHolding(
                        ...
                        context=record["context"][:300] + "..." if len(record["context"]) > 300 else record["context"]
                    )
    ...
    def find_cases_by_fact_pattern(
        ...
        if legal_context:
            chunks = [c for c in chunks if any(legal_context.lower() in tag.lower() for tag in c.source_document.legal_tags)]
    ...
    def generate_legal_research_memo(
        ...
        key_authorities = list(set([chunk.source_document.document_citation for chunk in chunks]))
        ...
        if client_facts:
            recommendations += f"Based on the client facts provided: {client_facts[:200]}...\n"
        recommendations += "Further research may be needed in areas with limited authorities.\n"
    ...
    def validate_legal_proposition(
        ...
        for chunk in chunks:
            ...
            supporting_keywords = ["held", "established", "confirmed", "affirmed", "correct"]
            opposing_keywords = ["rejected", "overruled", "distinguished", "incorrect", "wrong"]
            
            support_score = sum(1 for keyword in supporting_keywords if keyword in chunk_text)
            oppose_score = sum(1 for keyword in opposing_keywords if keyword in chunk_text)
            ...
            if support_score > oppose_score:
                supporting_authorities.append({
                    ...
                    "strength": "strong" if chunk.similarity_score > 0.8 else "moderate"
                })
        ...
        confidence_score = len(supporting_authorities) / total_authorities if total_authorities > 0 else 0

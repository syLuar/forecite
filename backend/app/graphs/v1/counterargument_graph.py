...
class CounterArgumentAnalysisOutput(BaseModel):
    ...
    counterarguments: List[CounterArgumentOutput] = Field(
        description="Generated counterarguments"
    )
    rebuttals: List[List[RebuttalOutput]] = Field(
        description="Rebuttals for each counterargument"
    )
...
async def rag_retrieval_node(state: CounterArgumentState) -> CounterArgumentState:
    ...
    for arg in key_arguments:
        if isinstance(arg, dict):
            ...
            primary_concepts.extend(
                [
                    concept.strip()
                    for concept in argument_text.split()
    ...
    for precedent in primary_precedents:
        try:
            ...
            for citation in citation_results[:5]:  # Limit results
                ...
                if any(
                    keyword in citation.get("text", "").lower()
                    for keyword in [
                        "distinguish",
                        "differ",
                        "inapplicable",
    ...
    for concept in primary_concepts[:3]:
        try:
            ...
            results = fulltext_search(search_terms=query, search_type="chunks", limit=8)
            alternative_interpretations.extend(results)
        except Exception as e:
            logger.warning(
                f"Fulltext search failed for alternative interpretations: {e}"
            )
    ...
    state["retrieved_counterargument_precedents"] = opposing_precedents[
        :15
    ...
    total_retrieved = (
        len(opposing_precedents)
        + len(distinguishing_factors)
        + len(alternative_interpretations)
        + len(procedural_challenges)
        + len(policy_arguments)
    )
    state["research_comprehensiveness"] = min(
        total_retrieved / 50.0, 1.0
    )  # Normalize to 0-1
...
async def vulnerability_analysis_node(
    ...
    for i, argument in enumerate(key_arguments):
        ...
        for precedent in retrieved_precedents[:5]:  # Limit analysis
            ...
            if any(
                keyword in precedent_text.lower()
        ...
        for factor in distinguishing_factors[:3]:
            ...
            if any(
                keyword in factor_text.lower()
                for keyword in ["distinguish", "different", "inapplicable"]
            ):
                factual_distinguishers.append(
                    f"Factual difference: {factor_text[:100]}"
                )
        ...
        for challenge in procedural_challenges[:3]:
            ...
            if any(
                keyword in challenge_text.lower()
...
async def counterargument_generation_node(
    state: CounterArgumentState,
) -> CounterArgumentState:
    """
...
Generate practical counterarguments that a skilled opposing counsel would actually raise."""
    ...
    try:
        ...
        avg_strength = (
            sum(ca.get("strength_assessment", 0.5) for ca in counterarguments)
            / len(counterarguments)
            if counterarguments
            else 0.5
        )
        ...
        for rebuttal_group in rebuttals:
            group_score = (
                sum(
                    0.7
                    if len(reb.get("content", "")) > 100
                    else 0.3 + 0.3
                    if len(reb.get("authority", "")) > 10
                    else 0.1
        ...
        state["rebuttal_quality"] = (
            sum(rebuttal_scores) / len(rebuttal_scores) if rebuttal_scores else 0.5
        )

        logger.info(
            f"Generated {len(counterarguments)} counterarguments with {sum(len(r) for r in rebuttals)} rebuttals"
        )
            ...
            fallback_counterarguments.append(
                {
                    ...
                    "factual_basis": text[:200]
                    if text
                    else "Factual analysis from case law",

...
class CounterArgumentSeedOutput(BaseModel):
    ...
    target_argument: int = Field(
        description="Index of argument being challenged (0-based)"
    )
...
class SearchQueryOutput(BaseModel):
    ...
    primary_query: str = Field(
        description="Primary search query focusing on opposing arguments and weaknesses"
    )
    alternative_query: str = Field(
        description="Alternative search query for broader counterargument research"
    )
    focus_areas: List[str] = Field(
        description="List of specific legal areas or concepts to focus on"
    )
...
class SearchExecutionOutput(BaseModel):
    ...
    unique_documents: int = Field(
        description="Number of unique documents after deduplication"
    )
...
class SingleCounterArgumentOutput(BaseModel):
    ...
    argument: str = Field(
        description="Detailed counterargument with specific legal reasoning"
    )
    supporting_authority: str = Field(
        description="Specific legal authority, case citation, or statute supporting this challenge"
    )
    factual_basis: str = Field(
        description="Factual foundation explaining why this challenge applies"
    )
...
class VulnerabilityAssessmentOutput(BaseModel):
    ...
    vulnerability_type: str = Field(
        description="Type of vulnerability: precedent, procedural, factual, or policy"
    )
...
async def query_generator_node(state: CounterArgumentState) -> CounterArgumentState:
    ...
    for i, arg in enumerate(key_arguments):
        if isinstance(arg, dict):
            ...
            arguments_summary.append(
                f"Argument {i + 1}: {arg_text[:200]}... Authority: {authority}"
            )
...
Case facts: {user_facts[:300]}..."""
    ...
    prompt_template = ChatPromptTemplate.from_messages(
        [
            ...
            HumanMessage(
                content="Generate targeted search queries to find counterarguments and opposing precedents."
            ),
        ]
    )
    ...
    try:
        ...
        alternative_query = (
            f"legal challenges factual distinguishers {user_facts.split()[:5]}"
        )
...
async def search_executor_node(state: CounterArgumentState) -> CounterArgumentState:
    ...
    try:
        ...
        primary_results = await vector_search(
            query_text=primary_query, jurisdiction=None, min_score=0.6, limit=15
        )
        retrieved_docs.extend(primary_results)

        # Alternative search with secondary query
        alt_results = await vector_search(
            query_text=alternative_query, jurisdiction=None, min_score=0.6, limit=10
        )
        ...
        for focus_area in focus_areas[:3]:  # Limit to top 3 focus areas
            focus_results = await vector_search(
                query_text=focus_area, jurisdiction=None, min_score=0.7, limit=5
            )
        ...
        state["retrieved_docs"] = unique_docs[
            :25
        ]  # Increased limit for counterargument research

        logger.info(
            f"Retrieved {len(unique_docs)} documents for counterargument analysis"
        )
...
async def vulnerability_analysis_node(
...
Identify ONE specific vulnerability in this argument."""
        ...
        try:
            vuln_output = await structured_llm.ainvoke(
            ...
            logger.info(
                f"Identified vulnerability in argument {i + 1}: {vuln_output.vulnerability_type}"
            )
    ...
    logger.info(
        f"Vulnerability analysis completed. Found {len(vulnerabilities)} vulnerabilities"
    )
...
async def challenge_identifier_node(
    state: CounterArgumentState,
) -> CounterArgumentState:
    """
...
Create a brief, specific description of how this vulnerability can be turned into a counterargument."""
        ...
        prompt_template = ChatPromptTemplate.from_messages(
            [
                SystemMessage(content=system_prompt),
                HumanMessage(
                    content="Create a counterargument seed based on this vulnerability."
                ),
            ]
        )

        try:
            seed_output = await structured_llm.ainvoke(
                prompt_template.format_messages()
            )
...
async def counterargument_developer_node(
    state: CounterArgumentState,
) -> CounterArgumentState:
    """
...
"""
    ...
    for i, seed in enumerate(seeds):
        ...
        target_argument = (
            key_arguments[target_arg] if target_arg < len(key_arguments) else {}
        )
        target_text = (
            target_argument.get("argument", "")
...
Write as if you are an experienced opposing counsel making this argument in court."""
    ...
    logger.info(
        f"Counterargument development completed. Generated {len(counterarguments)} counterarguments"
    )
    ...
    writer(
        {
            ...
            "description": f"Developed {len(counterarguments)} counterarguments: {', '.join(ca_titles[:2])}"
            + ("..." if len(ca_titles) > 2 else ""),
        }
    )
...
"""
    ...
    for i, ca in enumerate(counterarguments):
        ...
        original_text = (
            original_arg.get("argument", "") if isinstance(original_arg, dict) else ""
        )
        original_authority = (
            original_arg.get("supporting_authority", "")
...
Write as if you are defending your position in court against this challenge."""
        ...
        try:
            rebuttal_output = await structured_llm.ainvoke(
                prompt_template.format_messages()
            )
    ...
    logger.info(
        f"Rebuttal generation completed. Generated {len(all_rebuttals)} rebuttals"
    )
    ...
    writer(
        {
            ...
            "description": f"Generated {len(all_rebuttals)} rebuttal strategies: {', '.join(rebuttal_strategies[:2])}"
            + ("..." if len(rebuttal_strategies) > 2 else ""),
        }
    )

...
from app.core.llm import (
    get_llm_config_for_path,
    create_llm_from_config_path,
    create_llm,
)
...
class GraphLLMHelper:
    ...
    def get_node_llm(self, node_name: str) -> Any:
        ...
        if cache_key not in self._llm_cache:
            ...
            node_config_path = (
                f"{self.task_category}.{self.graph_name}.nodes.{node_name}"
            )
            ...
            try:
                llm = create_llm_from_config_path(
                    node_config_path, fallback_config=graph_config
                )
                logger.info(
                    f"Using node-specific LLM config for {self.graph_name}.{node_name}"
                )
            except Exception:
                if graph_config:
                    llm = create_llm(graph_config)
                    logger.info(
                        f"Using graph-level LLM config for {self.graph_name}.{node_name}"
                    )
                else:
                    raise RuntimeError(
                        f"No LLM configuration found for {self.graph_name}.{node_name}"
                    )
...
def create_graph_llm_helper(
    graph_name: str, task_category: str = "main"
) -> GraphLLMHelper:
    """

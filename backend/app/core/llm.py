...
def create_llm(
    model_config: Dict[str, Any], provider: Optional[str] = None
) -> BaseChatModel:
    """
    ...
    try:
        ...
        # Initialize the LLM
        logger.info(
            f"Initializing {provider} LLM with model: {provider_config.get('model', 'default')}"
        )
...
def create_custom_llm(
    model: str, provider: str = DEFAULT_PROVIDER, temperature: float = 0.2, **kwargs
) -> BaseChatModel:
    """
...
def get_llm_config_for_path(
    config_path: str, fallback_config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for part in path_parts:
            if part in config:
                ...
                if fallback_config:
                    logger.info(
                        f"Config path '{config_path}' not found, using fallback"
                    )
        ...
        if fallback_config:
            logger.warning(
                f"Error getting config for path '{config_path}': {str(e)}, using fallback"
            )
            return fallback_config.copy()
        else:
            raise LLMError(
                f"Failed to get LLM config for path '{config_path}': {str(e)}"
            ) from e

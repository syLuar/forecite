...
def create_postgresql_database():
    ...
    try:
        ...
        with default_engine.connect() as conn:
            ...
            if not result.fetchone():
                ...
                logger.info(f"PostgreSQL database already exists: {settings.postgres_db}")
    ...
    try:
        ...
        with target_engine.connect() as conn:
            ...
            try:
                conn.execute(text(f"GRANT CREATE ON SCHEMA public TO {settings.postgres_user}"))
                conn.execute(text(f"GRANT USAGE ON SCHEMA public TO {settings.postgres_user}"))
                conn.execute(text("COMMIT"))
                logger.info(f"Granted permissions to user: {settings.postgres_user}")
            except Exception as perm_error:
                logger.warning(f"Could not grant permissions (may already exist): {perm_error}")
                
    except Exception as e:
        logger.error(f"Failed to set up database permissions: {e}")
        logger.error("You may need to run the following SQL commands as a PostgreSQL superuser:")
...
def main():
    ...
    logger.info(f"Database URL: {database_url.replace(settings.postgres_password, '***') if settings.postgres_password else database_url}")
